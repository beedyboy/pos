  ...
  //show calculator
	...
	var pname = $(this).attr("name");
		...
		if(cid == "S"){
			 ...
			 var index  = localOrder.indexOf(product_id);
...
//get update
  ...
  $.ajax({
	...
	$(document).on('click', '.ord_type',  function(){ 
		var opt = $(this).attr("id");
		
		if(opt == "takein"){
			
				$('input#ord_type').val("Take-In");
				$(this).css("background","red");
					$("#takeout").css("background","#008000");
		}
		else{
					$('input#ord_type').val("Take-Out");
							$(this).css("background","red");
							$("#takein").css("background","#008000");
		}
	});
	...
	url:'actions.php',
	type: 'GET',
	data: ({invoice:invoice, cashier:cashier, ord_type:ord_type, date:date, action: action }), 
	success: function( result ){
