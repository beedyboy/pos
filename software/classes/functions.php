...
public static function loadLocalDistincts(){
$conn = Database::getInstance();
$select = $conn->db->prepare("SELECT * FROM products WHERE main !='C' AND main !='D' ");
$select->execute();
return $select;
} 
...
if($existCheck = self::existThree('products',  'main', 'subId',  'product_name', $_POST['main'], $_POST['subId'], $_POST['product_name'])==0)
{   
...
$category  = self::getColById('products', 'product_id', $_GET['product_id'], 4);
  //$ivat=self::getColById('products', 'product_id', $_GET['product_id'], 5);
 $price  = self::getColById('products', 'product_id', $_GET['product_id'], 6);
...
// $amount = round($amount- $discount);
 
	elseif($category=="S"): //soup
	  $qty =0; 
	$amount = 0;
		$price =0; 

  elseif($category=="SM"): //special meal
	  $qty =0;   
		$price =0; 
	$amount = $_GET['lamount']; 
...
if($GetSession->loggedin== TRUE):
  ...
  $invoice=$_POST['invoice'];
	...
	$resultas = self::amountSumSalesOrder($invoice);
				...
				$amount = $balance = $rowas['sum(amount)'];
 ...
 //return $_POST['tid']."-s- ".$_POST['seat']." -h-".$_POST['hid'];
...
$updateClass->bindParam(':main', $_POST['main'], PDO::PARAM_STR);
$updateClass->bindParam(':subId', $_POST['subId'], PDO::PARAM_STR);
